module Refinery
  module Admin
    class ResourcesController < ::Refinery::AdminController
      ...
      # webnam
      def find_all_resources(conditions='')
        @resources = Resource.where(conditions).where({:site_id => session[:current_site]}).order("updated_at DESC")
      end
      ...
      def create
        # webnam
        @resources = Resource.create_resources(params[:resource], session[:current_site])
    ...
    protected
      ...
      def paginate_resources(conditions={})
        #webnam
        @resources = Resource.where(conditions).where({:site_id => session[:current_site]}).
